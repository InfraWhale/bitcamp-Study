...
// JVM을 실행하는 동안 클래스가 로딩되면 같은 클래스에 대해 중복 로딩되지 않는다.
...
//    - JDK에서 제공하는 기본 라이브러리(jdk가 설치된 폴더의 하위 폴더인 /lib)에서 찾는다.
...
//     - 바이트 코드를 그대로 메모리에 두는 것이 아니라, 멤버의 종류에 따라 적절하게 분류한다.
