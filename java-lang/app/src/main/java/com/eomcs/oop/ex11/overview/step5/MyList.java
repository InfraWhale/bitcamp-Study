...
public class MyList {
  ...
  // - inner class 라고도 부른다.
  // - ListItorator가 사용할 바깥 클래스 MyList의 인스턴스를 주소를 저장할 필드와 생성자가 자동으로 추가된다.
  // - 기존의 static nested class 처럼 개발자가 직접 추가할 필요가 없다.
  // - 바깥 클래스의 인스턴스를 사용하는 경우, static nested class 로 만드는 것보다 더 편리하다.
  class ListIterator implements Iterator {
    ...
    public boolean hasNext() {
      return cursor < MyList.this.size(); // 바깥 클래스 객체 주소 사용
    }
