...
public class LinkedList {
  ...
  public void add(Object value) {
    ...
    if (head == null) {
      ...
      node.prev = tail;
      tail.next = node; // node.prev.next = node 라고 이해하면 되나?
      tail = node;
    }
    size++;
  }
  ...
  public void add(int index, Object value) {
    ...
    node.prev = newNode; // 현재 노드의 앞 노드로 새 노드로 설정
    newNode.next = node; // 새 노드의 다음 노드를 현재 노드로 설정.

    if (node == head) { // 첫 번쨰 노드라면
  ...
  private Node getNode(int index) {
    ...
    int count = 0;
    while (count < index) {
      node = node.next;// node는 객체의 주소를 담는 변수 // node.next는 node 주소로 찾아가서 거기 있는 변수
      count++;
    }
